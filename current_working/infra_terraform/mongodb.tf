...
data "aws_secretsmanager_secret" "mongosecret" {
...
resource "aws_instance" "mongodb" {
  ...
  user_data = <<-EOF
    ...
    # Retrieve credentials from AWS Secrets Manager using the imported secret
    max_attempts=10
    attempt=0
    while ! CREDENTIALS=$(aws secretsmanager get-secret-value --secret-id ${data.aws_secretsmanager_secret.mongosecret.id} --region ${var.region} --query SecretString --output text); do
      attempt=$((attempt + 1))
      if [ $attempt -eq $max_attempts ]; then
    ...
    CREDENTIALS=$(aws secretsmanager get-secret-value --secret-id ${data.aws_secretsmanager_secret.mongosecret.id} --region ${var.region} --query SecretString --output text)
    ...
    CREDENTIALS=$(/usr/local/bin/aws secretsmanager get-secret-value --secret-id ${data.aws_secretsmanager_secret.mongosecret.id} --region ${var.region} --query SecretString --output text)
